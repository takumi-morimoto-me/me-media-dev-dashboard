#!/usr/bin/env tsx
/**
 * 全ASPの月次スクレイパーを一括実行するスクリプト
 *
 * 使い方:
 * pnpm exec tsx src/scripts/run-all-monthly-scrapers.ts
 * pnpm exec tsx src/scripts/run-all-monthly-scrapers.ts --asp=a8net,moshimo
 */
...
async function runScraper(asp: string): Promise<ScraperResult> {
  ...
  try {
    // 動的にスクレイパーをインポート
    const scraperModule = await import(`./asp/monthly/${asp}/index`);
...
async function main() {
  ...
  for (let i = 0; i < targetAsps.length; i++) {
    ...
    const result = await runScraper(asp);
    results.push(result);

    // 次のASPの前に少し待機（サーバー負荷軽減）
    if (i < targetAsps.length - 1) {
      console.log('⏱️  次のASPまで5秒待機...');
