import { SmartcDailyScraper } from '../../daily/smartc/index';

/**
 * Smart-C 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/smartc/index.ts
 *
 * 環境変数:
 * - SMARTC_USERNAME
 * - SMARTC_PASSWORD
...
async function main() {
  ...
  const credentials = {
    username: process.env.SMARTC_USERNAME || '',
    password: process.env.SMARTC_PASSWORD || '',
  };
  ...
  const config = {
    ...
    aspId: process.env.SMARTC_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   SMARTC_USERNAME と SMARTC_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, SMARTC_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new SmartcDailyScraper(credentials, { ...config, month });
