import { {ASP_NAME}DailyScraper } from '../../daily/{asp-name}/index';

/**
 * {ASP_DISPLAY_NAME} 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/{asp-name}/index.ts
 *
 * 環境変数:
 * - {ASP_NAME}_USERNAME
 * - {ASP_NAME}_PASSWORD
...
async function main() {
  console.log('\n📋 {ASP_DISPLAY_NAME} 全期間データ取得');
  console.log('📅 2025年1月〜10月のデータを取得します\n');

  const credentials = {
    username: process.env.{ASP_NAME}_USERNAME || '',
    password: process.env.{ASP_NAME}_PASSWORD || '',
  };
  ...
  const config = {
    ...
    aspId: process.env.{ASP_NAME}_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   {ASP_NAME}_USERNAME と {ASP_NAME}_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, {ASP_NAME}_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new {ASP_NAME}DailyScraper(credentials, { ...config, month });
