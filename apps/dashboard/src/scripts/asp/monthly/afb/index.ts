import { AfbDailyScraper } from '../../daily/afb/index';

/**
 * afb 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/afb/index.ts
 *
 * 環境変数:
 * - AFB_USERNAME
 * - AFB_PASSWORD
...
async function main() {
  ...
  const config = {
    ...
    aspId: process.env.AFB_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   AFB_USERNAME と AFB_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, AFB_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new AfbDailyScraper(credentials, { ...config, month });
