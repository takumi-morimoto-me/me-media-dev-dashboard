import { RateladDailyScraper } from '../../daily/ratelad/index';

/**
 * Ratel AD 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/ratelad/index.ts
 *
 * 環境変数:
 * - RATELAD_USERNAME
 * - RATELAD_PASSWORD
...
async function main() {
  ...
  const credentials = {
    username: process.env.RATELAD_USERNAME || '',
    password: process.env.RATELAD_PASSWORD || '',
  };
  ...
  const config = {
    ...
    aspId: process.env.RATELAD_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   RATELAD_USERNAME と RATELAD_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, RATELAD_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new RateladDailyScraper(credentials, { ...config, month });
