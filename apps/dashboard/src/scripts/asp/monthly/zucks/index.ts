import { ZucksDailyScraper } from '../../daily/zucks/index';

/**
 * Zucks Affiliate 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/zucks/index.ts
 *
 * 環境変数:
 * - ZUCKS_USERNAME
 * - ZUCKS_PASSWORD
...
async function main() {
  console.log('\n📋 Zucks Affiliate 全期間データ取得');
  console.log('📅 2025年1月〜10月のデータを取得します\n');

  const credentials = {
    username: process.env.ZUCKS_USERNAME || '',
    password: process.env.ZUCKS_PASSWORD || '',
  };
  ...
  const config = {
    ...
    aspId: process.env.ZUCKS_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   ZUCKS_USERNAME と ZUCKS_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, ZUCKS_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new ZucksDailyScraper(credentials, { ...config, month });
