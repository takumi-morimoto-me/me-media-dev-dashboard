import { TgAffiliateDailyScraper } from '../../daily/tg-affiliate/index';

/**
 * TGアフィリエイト 月次スクレイパー (過去データ一括取得用)
 *
 * 使い方:
 * pnpm exec tsx src/scripts/asp/monthly/tg-affiliate/index.ts
 *
 * 環境変数:
 * - TG_AFFILIATE_USERNAME
 * - TG_AFFILIATE_PASSWORD
 * - RERE_MEDIA_ID
...
async function main() {
  ...
  const credentials = {
    username: process.env.TG_AFFILIATE_USERNAME || '',
    password: process.env.TG_AFFILIATE_PASSWORD || '',
  };
  ...
  const config = {
    ...
    aspId: process.env.TG_AFFILIATE_ASP_ID || '',
  };

  // 検証
  if (!credentials.username || !credentials.password) {
    console.error('❌ エラー: 認証情報が設定されていません');
    console.error('   TG_AFFILIATE_USERNAME と TG_AFFILIATE_PASSWORD を .env.local に設定してください');
    process.exit(1);
  }

  if (!config.mediaId || !config.accountItemId || !config.aspId) {
    console.error('❌ エラー: 必須の設定が不足しています');
    console.error('   RERE_MEDIA_ID, AFFILIATE_ACCOUNT_ITEM_ID, TG_AFFILIATE_ASP_ID を設定してください');
    process.exit(1);
  }
  ...
  for (const month of months) {
    ...
    const scraper = new TgAffiliateDailyScraper(credentials, { ...config, month });
