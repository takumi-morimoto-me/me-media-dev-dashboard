...
export class SmaADDailyScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private credentials: SmaADCredentials;
  private config: ScraperConfig;

  constructor(credentials: SmaADCredentials, config: ScraperConfig) {
  ...
  async login() {
    ...
    console.log('🔐 SmaADにログイン中...');

    // SmaADのログインページに移動
    await this.page.goto('https://smaad.net/login', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });

    await this.page.waitForTimeout(3000);
    await this.screenshot('smaad-login-page.png');
    ...
    await this.screenshot('smaad-before-login-click.png');
    ...
    await this.screenshot('smaad-after-login.png');
  ...
  async navigateToReportPage() {
    ...
    await this.screenshot('smaad-report-page.png');
  }
  ...
  async scrapeDailyData(): Promise<DailyData[]> {
    ...
    await this.page.waitForTimeout(2000);
    await this.screenshot('smaad-data-page.png');
...
async function main() {
  ...
  const { data: asp } = await supabase.from('asps').select('id').eq('name', 'SmaAD').single();
  ...
  if (!credentials?.username_secret_key) {
    console.error('SmaADの認証情報が取得できませんでした');
    return;
  }

  const scraper = new SmaADDailyScraper(
