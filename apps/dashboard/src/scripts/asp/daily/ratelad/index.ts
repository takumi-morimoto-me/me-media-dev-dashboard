...
export class RatelADDailyScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private credentials: RatelADCredentials;
  private config: ScraperConfig;

  constructor(credentials: RatelADCredentials, config: ScraperConfig) {
  ...
  async login() {
    ...
    console.log('🔐 Ratel ADにログイン中...');

    await this.page.goto('https://ratel-ad.com/', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });

    await this.page.waitForTimeout(3000);
    await this.screenshot('ratelad-login-page.png');
    ...
    await this.screenshot('ratelad-before-login-click.png');
    ...
    await this.screenshot('ratelad-after-login.png');
  ...
  async scrapeDailyData(): Promise<DailyData[]> {
    ...
    await this.screenshot('ratelad-data-page.png');
...
async function main() {
  ...
  const { data: asp } = await supabase.from('asps').select('id').eq('name', 'Ratel AD').single();
  ...
  if (!credentials?.username_secret_key) {
    console.error('Ratel ADの認証情報が取得できませんでした');
    return;
  }

  const scraper = new RatelADDailyScraper(
