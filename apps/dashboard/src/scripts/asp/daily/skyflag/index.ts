...
export class SKYFLAGDailyScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private credentials: SKYFLAGCredentials;
  private config: ScraperConfig;

  constructor(credentials: SKYFLAGCredentials, config: ScraperConfig) {
  ...
  async login() {
    ...
    console.log('🔐 SKYFLAGにログイン中...');

    // SKYFLAGのログインページに移動
    await this.page.goto('https://skyflag.info/login', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });

    await this.page.waitForTimeout(3000);
    await this.screenshot('skyflag-login-page.png');
    ...
    await this.screenshot('skyflag-before-login-click.png');
    ...
    await this.screenshot('skyflag-after-login.png');
  ...
  async navigateToReportPage() {
    ...
    for (const link of reportLinks) {
      ...
      if (text?.includes('レポート') || text?.includes('成果') || text?.includes('統計') ||
          href?.includes('report') || href?.includes('analytics')) {
    ...
    await this.screenshot('skyflag-report-page.png');
  }
  ...
  async scrapeDailyData(): Promise<DailyData[]> {
    ...
    await this.page.waitForTimeout(2000);
    await this.screenshot('skyflag-data-page.png');
...
async function main() {
  ...
  const { data: asp } = await supabase.from('asps').select('id').eq('name', 'SKYFLAG').single();
  ...
  if (!credentials?.username_secret_key) {
    console.error('SKYFLAGの認証情報が取得できませんでした');
    return;
  }

  const scraper = new SKYFLAGDailyScraper(
