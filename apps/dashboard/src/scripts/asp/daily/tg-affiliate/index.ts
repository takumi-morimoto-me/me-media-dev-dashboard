...
interface TGAffiliateCredentials {
  username: string;
  password: string;
}
...
export class TGAffiliateDailyScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private credentials: TGAffiliateCredentials;
  private config: ScraperConfig;

  constructor(credentials: TGAffiliateCredentials, config: ScraperConfig) {
  ...
  async login() {
    ...
    console.log('🔐 TGアフィリエイトにログイン中...');

    // TGアフィリエイトのログインページに移動
    await this.page.goto('https://www.tg-affiliate.com/', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });

    await this.page.waitForTimeout(3000);
    await this.screenshot('tg-login-page.png');

    // ログインフォームを探す
    console.log('ログインフォームを探しています...');

    // ログインIDとパスワードのフィールドを探す
    const loginIdInput = this.page.locator('input[name="loginId"], input[name="login_id"], input[type="text"]').first();
    ...
    await this.screenshot('tg-before-login-click.png');
    ...
    await this.screenshot('tg-after-login.png');
  ...
  async navigateToReportPage() {
    ...
    await this.page.waitForTimeout(3000);
    await this.screenshot('tg-report-page.png');
  ...
  async scrapeDailyData(): Promise<DailyData[]> {
    ...
    await this.page.waitForTimeout(2000);
    await this.screenshot('tg-before-search.png');
    ...
    for (const button of buttons) {
      ...
      if (text?.includes('表示') || text?.includes('検索') || value?.includes('表示') || value?.includes('検索')) {
        ...
        await button.click();
        await this.page.waitForTimeout(5000);
        await this.screenshot('tg-after-search.png');
        break;
      }
    }
    ...
    await this.screenshot('tg-data-final.png');
...
async function main() {
  ...
  if (!credentials) {
    console.error('TGアフィリエイトの認証情報が取得できませんでした');
    return;
  }
  ...
  const scraper = new TGAffiliateDailyScraper(
    {
