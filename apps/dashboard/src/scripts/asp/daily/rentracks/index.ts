...
interface RentracksCredentials {
  username: string;
  password: string;
}
...
export class RentracksDailyScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private credentials: RentracksCredentials;
  private config: ScraperConfig;

  constructor(credentials: RentracksCredentials, config: ScraperConfig) {
  ...
  async login() {
    ...
    console.log('🔐 レントラックスにログイン中...');

    // レントラックスのログインページに移動
    await this.page.goto('https://www.rentracks.jp/adman/login.html', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });

    await this.page.waitForTimeout(3000);
    await this.screenshot('rentracks-login-page.png');

    // ログインフォームを探す
    console.log('ログインフォームを探しています...');

    // ログインIDとパスワードのフィールドを探す
    const loginIdInput = this.page.locator('input[name="login_id"], input[name="loginId"], input[type="text"]').first();
    ...
    await this.screenshot('rentracks-before-login-click.png');
    ...
    await this.screenshot('rentracks-after-login.png');
  ...
  async navigateToReportPage() {
    ...
    await this.page.waitForTimeout(3000);
    await this.screenshot('rentracks-report-page.png');
  ...
  async scrapeDailyData(): Promise<DailyData[]> {
    ...
    await this.page.waitForTimeout(2000);
    await this.screenshot('rentracks-before-search.png');
    ...
    for (const button of buttons) {
      ...
      if (text?.includes('表示') || text?.includes('検索') || value?.includes('表示') || value?.includes('検索')) {
        ...
        await button.click();
        await this.page.waitForTimeout(5000);
        await this.screenshot('rentracks-after-search.png');
        break;
      }
    }
    ...
    await this.screenshot('rentracks-data-final.png');
...
async function main() {
  ...
  if (!credentials) {
    console.error('レントラックスの認証情報が取得できませんでした');
    return;
  }
  ...
  const scraper = new RentracksDailyScraper(
    {
