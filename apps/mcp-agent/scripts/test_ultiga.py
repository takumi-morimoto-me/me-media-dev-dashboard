"""Ultiga (アルテガアフィリエイト) を直接Playwrightでテストするスクリプト"""
...
def test_ultiga():
    with sync_playwright() as p:
        ...
        try:
            print('1. Ultiga ログインページにアクセス')
            page.goto('https://ultiga.jp/', wait_until='domcontentloaded', timeout=60000)
            time.sleep(3)
            page.screenshot(path='screenshots/ultiga_test_1_initial_page.png')
            ...
            if login_links > 0:
                ...
                time.sleep(3)
                page.screenshot(path='screenshots/ultiga_test_2_login_page.png')
            ...
            if login_inputs > 0 and password_inputs > 0:
                ...
                page.locator('input[type="email"], input[type="text"], input[name*="email"], input[name*="login"]').first.fill(
                    os.getenv('ULTIGA_USERNAME', 'test')
                )
                print('   ログインID入力完了')
                time.sleep(0.5)

                page.locator('input[type="password"]').first.fill(
                    os.getenv('ULTIGA_PASSWORD', 'test')
                )
                print('   パスワード入力完了')
                time.sleep(0.5)
                page.screenshot(path='screenshots/ultiga_test_3_credentials_filled.png')
                ...
                if login_buttons > 0:
                    ...
                    time.sleep(5)
                    page.screenshot(path='screenshots/ultiga_test_4_after_login.png')
                    ...
                    if report_links > 0:
                        ...
                        time.sleep(3)
                        page.screenshot(path='screenshots/ultiga_test_5_report_page.png')
            ...
            print(f'\n❌ エラー: {e}')
            page.screenshot(path='screenshots/ultiga_test_error.png')
            import traceback
            traceback.print_exc()
