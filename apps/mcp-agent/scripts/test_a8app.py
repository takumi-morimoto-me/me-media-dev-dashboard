"""A8app (SeedApp) を直接Playwrightでテストするスクリプト"""
...
def test_a8app():
    with sync_playwright() as p:
        ...
        try:
            print('1. A8app (SeedApp) ログインページにアクセス')
            page.goto('https://admin.seedapp.jp/', wait_until='domcontentloaded', timeout=60000)
            time.sleep(3)
            page.screenshot(path='screenshots/a8app_test_1_login_page.png')
            ...
            if login_inputs > 0 and password_inputs > 0:
                ...
                page.locator('input[type="email"], input[type="text"], input[name="email"]').first.fill(
                    os.getenv('A8APP_USERNAME', 'test')
                )
                print('   ログインID入力完了')
                time.sleep(0.5)

                page.locator('input[type="password"]').first.fill(
                    os.getenv('A8APP_PASSWORD', 'test')
                )
                print('   パスワード入力完了')
                time.sleep(0.5)
                page.screenshot(path='screenshots/a8app_test_2_credentials_filled.png')
                ...
                time.sleep(5)
                page.screenshot(path='screenshots/a8app_test_3_after_login.png')

                print(f'5. ログイン後のURL: {page.url}')

                print('6. 日別レポートリンクをクリック')
                daily_report_links = page.locator('a:has-text("日別レポート")').count()
                print(f'   日別レポートリンク数: {daily_report_links}')

                if daily_report_links > 0:
                    page.locator('a:has-text("日別レポート")').click()
                    time.sleep(3)
                    page.screenshot(path='screenshots/a8app_test_4_daily_report.png')

                    print(f'7. 最終URL: {page.url}')
            ...
            print(f'\n❌ エラー: {e}')
            page.screenshot(path='screenshots/a8app_test_error.png')
            import traceback
            traceback.print_exc()
