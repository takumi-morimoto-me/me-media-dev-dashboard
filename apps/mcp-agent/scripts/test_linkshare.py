"""LinkShare (リンクシェア) を直接Playwrightでテストするスクリプト"""
...
def test_linkshare():
    with sync_playwright() as p:
        ...
        try:
            print('1. LinkShare ログインページにアクセス')
            page.goto('https://cli.linksynergy.com/cli/publisher/home.php', wait_until='domcontentloaded', timeout=60000)
            time.sleep(3)
            page.screenshot(path='screenshots/linkshare_test_1_login_page.png')
            ...
            if login_inputs > 0 and password_inputs > 0:
                ...
                page.locator('input[type="email"], input[type="text"], input[name*="email"], input[name*="login"]').first.fill(
                    os.getenv('LINKSHARE_USERNAME', 'test')
                )
                print('   ログインID入力完了')
                time.sleep(0.5)

                page.locator('input[type="password"]').first.fill(
                    os.getenv('LINKSHARE_PASSWORD', 'test')
                )
                print('   パスワード入力完了')
                time.sleep(0.5)
                page.screenshot(path='screenshots/linkshare_test_2_credentials_filled.png')

                print('4. ログインボタンをクリック')
                login_buttons = page.locator('button[type="submit"], input[type="submit"], button:has-text("ログイン"), button:has-text("Sign In")').count()
                print(f'   ログインボタン数: {login_buttons}')

                if login_buttons > 0:
                    page.locator('button[type="submit"], input[type="submit"], button:has-text("ログイン"), button:has-text("Sign In")').first.click()
                    time.sleep(5)
                    page.screenshot(path='screenshots/linkshare_test_3_after_login.png')
                    ...
                    if report_links > 0:
                        ...
                        time.sleep(3)
                        page.screenshot(path='screenshots/linkshare_test_4_report_page.png')
            ...
            print(f'\n❌ エラー: {e}')
            page.screenshot(path='screenshots/linkshare_test_error.png')
            import traceback
            traceback.print_exc()
