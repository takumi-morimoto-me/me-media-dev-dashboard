...
def run_scraper():
    """Link-AG 日次データ取得スクレイパー"""
    
    SUPABASE_URL = os.getenv('SUPABASE_URL')
    SUPABASE_KEY = os.getenv('SUPABASE_SERVICE_ROLE_KEY')
    client = create_client(SUPABASE_URL, SUPABASE_KEY)
    
    username = os.getenv('LINKAG_USERNAME')
    password = os.getenv('LINKAG_PASSWORD')
    
    if not username or not password:
        return {"success": False, "records_saved": 0, "error": "Missing credentials"}
    
    asp_response = client.table('asps').select('id').eq('name', 'Link-AG（ビギナーズ）').execute()
    ...
    with sync_playwright() as p:
        ...
        try:
            print("Logging in to Link-AG...")
            page.goto("https://link-ag.net/")
