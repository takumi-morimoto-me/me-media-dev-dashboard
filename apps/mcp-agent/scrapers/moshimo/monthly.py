...
def _run_scraper_attempt(settings, username, password, headless):
    ...
    download_dir = Path(f"/tmp/moshimo_downloads_{int(time.time())}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                page.screenshot(path="/tmp/moshimo_error.png")
            except:
                pass
            ...
            try:
                supabase.table('asps').update({
                    ...
                    'scrape_notes': str(e)[:500]
                }).eq('id', asp_id).execute()
            except Exception as update_err:
                print(f"Note: Could not update ASP status: {update_err}")
