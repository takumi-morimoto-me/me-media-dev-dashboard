-- Fix get_asp_monthly_data to only use actuals table
-- The previous version (015) incorrectly combined both actuals and daily_actuals,
-- causing duplicate data when both tables contain the same information
...
BEGIN
    ...
    RETURN QUERY
    -- Only use actuals table (monthly data)
    -- daily_actuals should only be used by get_asp_daily_data function
    SELECT
