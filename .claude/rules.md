# Claude 作業ルール (me-media-dev-dashboard)

このプロジェクトで Claude が開発を行う際のルールを以下に定めます。

## 0. **絶対遵守事項**

このファイル（CLAUDE.md）は、Claude Code起動時に**必ず自動で読み込まれます**。
以下のルールは**絶対に守る**必要があります：

### **開発フェーズにおける認証・RLSの扱い**
-   **`docs/04_development/development-guidelines.md`に明記されている通り、開発フェーズでは認証（Supabase Authentication）およびアクセス制御（RLS: Row Level Security）を実装しません。**
-   データベース操作において、RLSポリシーの作成や有効化を**絶対に行わないでください**。
-   開発中は、全ての機能・データに認証なしでアクセス可能とします。
-   本番リリース前に、別途認証・RLSを実装します。
...
-   作業を開始する前に、必ず `docs` ディレクトリ内の関連ドキュメントを確認してください。ドキュメントは以下のカテゴリに分類されています。
    ...
    -   **`docs/04_development/development-guidelines.md`**: 開発ガイドライン（認証・RLSなどの重要な制約を含む）
...
## 8. **Claude 固有のベストプラクティス**

### **8.1. TodoWrite ツールの活用**
-   複数ステップのタスクや複雑な実装では、積極的に TodoWrite ツールを使用してタスクを管理してください。
-   進捗を可視化し、完了したタスクは即座にマークしてください。

### **8.2. 並列実行の最適化**
-   独立した複数のファイル読み込みや検索は、可能な限り並列で実行してください。
-   依存関係がある処理のみ、順次実行してください。

### **8.3. コミュニケーションスタイル**
-   簡潔で直接的な回答を心がけてください。
-   不要な前置きや後置きの説明は避けてください。
-   ユーザーが明示的に詳細を求めた場合のみ、詳しい説明を提供してください。

---
このルールは、Claude が最高のパフォーマンスを発揮し、プロジェクトの持続可能な成長に貢献するためのものです。
