...
const ResultsPage: React.FC<ResultsPageProps> = ({ mediaNames, fiscalYearStartMonth, budgetItems, setBudgetItems }) => {
  ...
  const [previousResultsData, setPreviousResultsData] = useState<DailyBudgetData | null>(null);
  const [previousYearlyResultsData, setPreviousYearlyResultsData] = useState<MonthlyBudgetData | null>(null);
        ...
        const fetchYearlyData = async () => {
            try {
                ...
                const currentPeriodPromises = monthsToFetch.map(monthNum => {
                    ...
                    return getResultsData(selectedMedia, calendarYear, calendarMonth);
                });
                ...
                setPreviousYearlyResultsData(allPreviousMonthsData);

                setResultsData(null);
                setPreviousResultsData(null);
            } catch (error) {
        ...
        const fetchMonthlyData = async () => {
            try {
                ...
                const [currentData, previousData] = await Promise.all([
                    getResultsData(selectedMedia, year, month),
                    getResultsData(selectedMedia, year - 1, month)
                ]);
                setResultsData(currentData);
                setPreviousResultsData(previousData);
                setYearlyResultsData(null);
                setPreviousYearlyResultsData(null);
            } catch (error) {
  ...
  }, [resultsData, granularity, targetDate, calculateParentTotals, daysInMonth]);

   const calculatedPreviousDailyData = useMemo(() => {
    if (granularity !== '日次' || !previousResultsData) return null;
    return calculateParentTotals(previousResultsData, daysInMonth);
  }, [previousResultsData, granularity, targetDate, calculateParentTotals, daysInMonth]);
  ...
  }, [yearlyResultsData, granularity, fiscalYearStartMonth, calculateParentTotals, selectedFiscalPeriod]);
  
  const calculatedPreviousYearlyData = useMemo(() => {
    if (granularity !== '月次' || !previousYearlyResultsData) return null;
    const calculatedData: MonthlyBudgetData = {};
    const fiscalMonths = getMonthsForFiscalYear(fiscalYearStartMonth);
    for (const month of fiscalMonths) {
        const monthData = previousYearlyResultsData[month];
        if (monthData) {
    ...
    return calculatedData;
  }, [previousYearlyResultsData, granularity, fiscalYearStartMonth, calculateParentTotals, selectedFiscalPeriod]);
                 ...
                 {flatItems.map(item => {
                  ...
                  <tr key={item.id} className={`group ${item.isHeader ? 'bg-slate-100 dark:bg-slate-800' : 'bg-white dark:bg-dark-background hover:bg-slate-50/50 dark:hover:bg-slate-700/50'}`}>
                    ...
                    {granularity === '日次' && Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                      ...
                      const currentValue = getDayValue(item.isEditable ? resultsData : calculatedDailyData, item.id, day);
                      const previousValue = getDayValue(item.isEditable ? previousResultsData : calculatedPreviousDailyData, item.id, day);
                      const yoy = calculateYoY(currentValue, previousValue);
